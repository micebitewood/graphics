...
public class Geometry
{
	...
	public void draw(Graphics g, Projection projection)
	{
		...
		for(int i = 0; i < faces.length; i++)
		{
			...
			double area = 0;
			area += (point[0][0] - point[1][0]) * (point[0][1] + point[1][1]) / 2;
			area += (point[1][0] - point[2][0]) * (point[1][1] + point[2][1]) / 2;
			area += (point[2][0] - point[3][0]) * (point[2][1] + point[3][1]) / 2;
			area += (point[3][0] - point[0][0]) * (point[3][1] + point[0][1]) / 2;
			if(area < 0)
				return;
			for(int j = 0; j < face.length; j++)
			{
